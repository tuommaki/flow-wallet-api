...
func Test_TransactionSignByAdmin(t *testing.T) {
	...
	tx, err := txSvc.Sign(context.Background(), cfg.AdminAddress, "", nil, transactions.General)
	if err != nil {
...
func Test_TransactionSignByAnotherAccount(t *testing.T) {
	...
	tx, err := svcs.GetTransactions().Sign(ctx, acc.Address, "", nil, transactions.General)
	if err != nil {
		t.Fatalf("expected err == nil, got %#v", err)
	}
